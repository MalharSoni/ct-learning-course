...
import { v5FoundationTest, assessmentAutoGradedPoints } from '@/lib/assessment-data';
...
export default function ResultsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const params = new URLSearchParams({ assessment: ASSESSMENT_ID });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avg: Math.round((avg / autoMax) * 100),
      passed,
  ...
  const exportHref = `/api/assessments/results/export?assessment=${ASSESSMENT_ID}${
  ...
  return (
    <>
      <Sidebar />
      <Topbar title="Unit Test Results" />
      <main className="ml-[280px] min-h-screen transition-colors" role="main">
        <div className="mx-auto max-w-5xl px-6 py-10 lg:px-8 lg:py-12 space-y-5">
          <div>
            <h1 className="text-[26px] font-black tracking-tight">Unit Test Results</h1>
            <p className="text-[14px] text-muted-foreground mt-1">
              {test.title} — auto-graded multiple-choice scores ({autoMax} pts). Short
              answers are graded by hand and are not scored here.
            </p>
          </div>
          ...
          {submitted && submitted.length === 0 && (
            <Card>
              <CardContent className="pt-6 text-[14px] text-muted-foreground">
                No submissions yet. Scores will appear here as students submit the test.
              </CardContent>
            </Card>
          )}
          ...
          <p className="text-[12px] text-muted-foreground">
            &ldquo;Result (auto)&rdquo; compares multiple-choice points to the {test.passingScore}-point
            cutoff. Final pass/fail also depends on the {test.totalPoints - autoMax} short-answer points you
            grade by hand.
          </p>
        </div>
      </main>
